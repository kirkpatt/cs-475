...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Taylor Kirkpatrick" };
...
2^16	65536
*/
const int NUM_PARTICLES = 1024* 2048;
const int LOCAL_SIZE    = 32;
const char *CL_FILE_NAME   = { "kirkpatt.cl" };
...
Display( )
{
	...
	Frame++;
	char str[128];
	sprintf( str, "Frame %8d -- %6.5f GigaParticles/Sec", Frame, (float)NUM_PARTICLES/ElapsedTime/1000000000. );
	if ((Frame % 10) == 0)
		WritePerf(ElapsedTime);
...
WriteFrame( )
{
	if (Frame <= 500) {
		char filename[128];
		sprintf(filename, "kirkpatt.%03d.particles", Frame); // use your engr login instead
...
void WritePerf(double timerecord) {
	if (Frame <= 500) {
		FILE *fp = fopen("kirkpatt.perf", "a");
		fprintf(fp, "%10d,%5d,%6.5f\n", NUM_PARTICLES, Frame, (float)NUM_PARTICLES / timerecord / 1000000000.);
		fclose(fp);
	}
}
